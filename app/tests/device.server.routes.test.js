...
describe('Device CRUD tests', function() {
					...
					.end(function(deviceSaveErr) {
						...
						agent.get('/devices')
							.end(function(devicesGetErr, devicesGetRes) {
								...
								(devices[0].manufacturer).should.match('Device Name');
								(devices[0].ip).should.match('1.1.1.1');
	...
	it('should not be able to save Device instance if no manufacturer is provided', function(done) {
		// Invalidate manufacturer field
		device.manufacturer = '';
			...
			.end(function(signinErr) {
				...
				agent.post('/devices')
					...
					.end(function(deviceSaveErr, deviceSaveRes) {
						...
						// Update Device name
						device.manufacturer = 'WHY YOU GOTTA BE SO MEAN?';
						...
						agent.put('/devices/' + deviceSaveRes.body._id)
							...
							.end(function(deviceUpdateErr, deviceUpdateRes) {
								...
								if (deviceUpdateErr) {
									done(deviceUpdateErr);
								}

								// Set assertions
								(deviceUpdateRes.body._id).should.equal(deviceSaveRes.body._id);
								(deviceUpdateRes.body.manufacturer).should.match('WHY YOU GOTTA BE SO MEAN?');
